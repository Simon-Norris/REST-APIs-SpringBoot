...
public class CashCardController {
    ...
    @PostMapping
    public ResponseEntity<Void> createCashCard(@RequestBody CashCard cashCardRequest, UriComponentsBuilder ucb, Principal principal){
        CashCard cashCardWithOwner = new CashCard(null, cashCardRequest.amount(), principal.getName());
        CashCard cashCard = cashCardRepository.save(cashCardWithOwner);
    ...
    private ResponseEntity<Void> deleteCard(@PathVariable Long requestedId, Principal principal) {
        boolean recordExists = cashCardRepository.existsByIdAndOwner(requestedId, principal.getName());
        if (recordExists) {
